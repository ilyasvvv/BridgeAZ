...
export default function Landing() {
  return (
    <div className="flex flex-col gap-16">
      ...
      <header className="sticky top-0 z-20 border-b border-border/60 bg-white/70 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3 md:px-12">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/join"
              className="rounded-full bg-coral px-5 py-2 text-xs font-semibold uppercase tracking-wide text-white"
            >
              Join
            </Link>
          </div>
      ...
      <section className="relative overflow-hidden bg-[#0B1628] px-6 py-16 md:px-12">
      ...
      >
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_20%_0%,rgba(21,101,163,0.06)_0%,transparent_50%)]" />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_80%_100%,rgba(212,118,60,0.06)_0%,transparent_50%)]" />
        <div className="relative mx-auto max-w-6xl px-6 md:px-12">
          ...
          {testimonials.map((item) => (
            <div key={item.name} className="rounded-2xl border border-white/60 bg-white/80 p-6 shadow-elevated ring-1 ring-white/30 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-glow hover:ring-brand/10">
              <p className="text-sm leading-relaxed text-mist">"{item.quote}"</p>
              <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-teal">{item.name}</p>
            </div>
          ))}
          </div>
        </div>
            ...
            <div>
              ...
              <ul className="mt-4 space-y-3">
                {footerLinks.platform.map((link) => (
                  <li key={link.label}>
                    <Link to={link.href} className="text-sm text-mist transition-colors hover:text-brand">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="mt-14 border-t border-border/60 pt-8">
            <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
              ...
              <div className="flex items-center gap-6">
                <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="text-mist transition-colors hover:text-brand" aria-label="Twitter">
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-mist transition-colors hover:text-brand" aria-label="LinkedIn">
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="text-mist transition-colors hover:text-brand" aria-label="Instagram">
